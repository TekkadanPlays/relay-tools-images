...
# Build cert-domains.txt — certrenew.sh reads this for issue/renewal
# Core domains are always included; optional services append theirs below
cat << EOF > /srv/haproxy/cert-domains.txt
# Auto-generated by configure.sh — $(date -Iseconds)
# Add domains here and run: systemctl start certrenew.service
$MYDOMAIN
app.$MYDOMAIN
monitor.$MYDOMAIN
EOF
...
# Build -d flags from the domain list
CERT_DOMAINS=""
while IFS= read -r line; do
    line=$(echo "$line" | sed 's/#.*//' | xargs)
    [ -n "$line" ] && CERT_DOMAINS="$CERT_DOMAINS -d $line"
done < /srv/haproxy/cert-domains.txt
...
EOF
    ...
    echo "Sporeboard started at sporeboard.$MYDOMAIN (port 3400)"
fi
...
if [ "${SPORECHAT_ENABLED:-false}" = "true" ]; then
    ...
    mkdir -p /srv/sporechat/spore /srv/sporechat/jam
...
EOF
    # Register subdomain for TLS cert
    grep -qxF "meet.$MYDOMAIN" /srv/haproxy/cert-domains.txt 2>/dev/null || echo "meet.$MYDOMAIN" >> /srv/haproxy/cert-domains.txt
    machinectl start sporechat
    echo "Sporechat started at meet.$MYDOMAIN (port 3500)"
fi
